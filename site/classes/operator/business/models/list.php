...

{
	...
	function getShop_name($shop_id){
		...
		function getList_a($start,$per_page,$where='1',$order='1')
	{
		$list = null;
		...
		$sql="SELECT a.*,b.settle_totalKm,b.settle_losses,b.settle_end,b.settle_overKmMoney,b.settle_overKmMoney_have,".
			 ...
			 "h.shunt_endtimes,h.shunt_other,j.bro_name,k.settle_favor as settle_favor_m,k.settle_infact,k.settle_billMoney_have,m.shop_name,n.emp_name AS jiedaiyuan ".
		...
		if( ($result = $this->_db->query($sql)) )
		{
			while ( ($row = $this->_db->fetchrow($result)) )
			{
						...
						if ($row['paiche_type']==1 || $row['paiche_type']==2){//临时
							if ($row['settle_totalKm']==0){//未还车
								...
								if ($row['settle_losses']==2){
									$row["paiche_status_color"]="#FFFFFF";//已结账
									$row["paiche_font_color"]="#000000";
				...
				//显示提醒信息
				if (($row['paiche_type']==1 || $row['paiche_type']==2) && $row['paiche_endDate']<time() && $row['settle_totalKm']==0 && $row['paiche_status']==1 ){
					$row["show_tip"]=1;
				}
				$row['paiche_startDate2'] = $row['paiche_startDate']>0 ? date("Y-m-d",$row['paiche_startDate']) :"-";
				$row['paiche_endDate2'] = $row['paiche_endDate']>0 ? date("Y-m-d",$row['paiche_endDate']) : "—";
				...
				$row['paiche_AccountendDate'] = $row['paiche_AccountendDate']>0 ? date("Y-m-d",$row['paiche_AccountendDate']) : "";
				$row['paiche_dispatchTimes'] = $row['paiche_dispatchTimes']>0 ? date("Y-m-d",$row['paiche_dispatchTimes']) : "—";
				$row['paiche_checkTimes'] = $row['paiche_checkTimes']>0 ? date("Y-m-d H:i:s",$row['paiche_checkTimes']) : "—";
				$row['shunt_endtimes'] = $row['shunt_endtimes']>0 ? date("Y-m-d",$row['shunt_endtimes']) : "";
		...
		if(($result = $this->_db->query($sql)))
        {
            while ( ($row = $this->_db->fetchrow($result)) )
            {
            	...
            	if (isset($row['paiche_startDate'])){
					...
					$row['paiche_youhuilist']=$this->getYouhui($row['paiche_id']);
