<?php
class Modelzijia_linshi extends Model

{
	...
	function getyhwj(){
		$list=null;
		$sql="select * from paiche  where  paiche_type=1 and paiche_jie=2";
	...
	function getyhwj_b($paicheCounterShop){
		$list=null;
		$sql="select * from paiche  where paicheCounterShop={$paicheCounterShop} and paiche_type=1 and paiche_jie=2";
		if(($result = $this->_db->query($sql)))
        {
            while ( ($row = $this->_db->fetchrow($result)) )
            {
	...
	function getzuijinyueyue(){
		$list=null;
		$shijian=strtotime(date('Y-m-d'));
        $shijian_a=strtotime(date('Y-m-d',strtotime('+3 day')));
		$sql="select * from paiche  where  paiche_type=1 and paiche_jie=-1 and paiche_startDate>{$shijian} and paiche_startDate<{$shijian_a}";
		if(($result = $this->_db->query($sql)))
        {
            while ( ($row = $this->_db->fetchrow($result)) )
            {
	...
	function getzuijinyueyue_b($paicheCounterShop){
		$list=null;
		$shijian=strtotime(date('Y-m-d'));
        $shijian_a=strtotime(date('Y-m-d',strtotime('+3 day')));
		$sql="select * from paiche  where paicheCounterShop={$paicheCounterShop} and paiche_type=1 and paiche_jie=-1 and paiche_startDate>{$shijian} and paiche_startDate<{$shijian_a}";
		if(($result = $this->_db->query($sql)))
        {
            while ( ($row = $this->_db->fetchrow($result)) )
            {
	...
	function getpingtai ($id){
		$sql="SELECT name from pintai where id={$id}";
		$list=null;
		if(($result = $this->_db->query($sql)))
        {
            while ( ($row = $this->_db->fetchrow($result)) )
	...
	function getList($start,$per_page,$where='1',$order='1')
	{
		...
		if( ($result = $this->_db->query($sql)) )
		{
			while ( ($row = $this->_db->fetchrow($result)) )
			{		
					if ($_SESSION['shopid']!=$row['paiche_shopid']&&$row['paicheCounterShop']!=$_SESSION['shopid']){
						$row['paiche_linkerPhone']=$this->hidchar($row['paiche_linkerPhone'],'','');
						$row['paiche_linker']=$this->hidchar('',$row['paiche_linker'],'');
						$row['paiche_linkerNum']=$this->hidchar('','',$row['paiche_linkerNum']);
				...
				//实际收款
				$row['paiche_Accountlist']=$this->getAccount($row['paiche_id']);
	...
	function getAccount($pid){
		$list = null;
		$sql="SELECT money from account_log where paiche_id={$pid} and bill_type<>4 and bill_type<>1 order by add_time desc";
	...
	function get_chaoshiweihuan_a(){
		$list=null;
		$sql="select * from paiche  where paiche_jie=1 and paiche_type=1 and paiche_endDate < ".(time()-60*60*4);
		if(($result = $this->_db->query($sql)))
        {
            while ( ($row = $this->_db->fetchrow($result)) )
            {
	...
	function get_chaoshiweihuan_b($paicheCounterShop){
		$list=null;
		$sql="select * from paiche  where paicheCounterShop={$paicheCounterShop} and paiche_jie=1 and paiche_type=1 and paiche_endDate < ".(time()-60*60*4);
		if(($result = $this->_db->query($sql)))
        {
            while ( ($row = $this->_db->fetchrow($result)) )
            {
	...
	function getBusinessInfo($uid,$single=0)
	{
		...
		$sql="SELECT a.*,b.*,".
			 "c.emp_name AS yewuyuan,c.emp_name as emp_namea,c.emp_phone AS yewuyuan_phone,cc.shop_name as shop_namea,d.emp_name AS siji,d.emp_phone,e.car_num,e.car_color,e.car_remarks_a,e.car_types,e.car_remarks_b,e.car_remarks,e.car_brand,e.car_model,e.car_motor,e.car_price,ee.car_num as car_num2,f.client_name,f.client_balance,h.shunt_rent,h.shunt_rented,h.shunt_other,h.shunt_endcode,h.shunt_money,j.bro_name,".
			 "kk.total_conv_money,kk.total_in_money,mm.total_in_money2,nn.total_month_money,m.shop_name,m.shop_phone,n.emp_name AS jiedaiyuan,z.emp_name as shenhe_empname ".
	...
	function getChargeList($pid,$where='',$pids='')
	{
		...
		if(($result = $this->_db->query($sql)))
        {
            while ( ($row = $this->_db->fetchrow($result)) )
            {
            	...
            	$row['addtime'] = $row['addtime']>0 ? date("Y-m-d H:i",$row['addtime']) :"";
            	if($row['addemp']){
            		$row['emp_name'] =$this->get_emp($row['addemp']);
            		$row['emp_name']=$row['emp_name']['emp_name'];
            	}else{
            		$row['emp_name'] ="";
            	}
	...
	function getChangList($pid,$where='',$order="DESC")
	{
		$list = null;
		$sql = "SELECT a.*,b.car_num as carA,c.car_num as carB FROM `changecar` AS a LEFT JOIN `car` AS b ON a.changecar_carA=b.car_id LEFT JOIN `car` AS c ON a.changecar_carB=c.car_id WHERE a.`changecarPaicheId`={$pid} {$where} order by changecar_times {$order}";
		//print_r($sql);exit;
		if(($result = $this->_db->query($sql)))
        {
            while ( ($row = $this->_db->fetchrow($result)) )
	...
	//优惠金额
	function getYouhui($paiche_id){
		$list=null;
		$sql="SELECT a.*,b.emp_name from paiche_youhui as a left join emp as b on a.youhui_empid=b.emp_id where paiche_id={$paiche_id}";
		if(($result = $this->_db->query($sql)))
        {
            while ( ($row = $this->_db->fetchrow($result)) )
            {
            	$row['youhui_addtime'] = $row['youhui_addtime']>0 ? date("Y-m-d H:i:s",$row['youhui_addtime']) : "—";
				$list[] = $row;
