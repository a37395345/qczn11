 ...
 * 
 * history:
 * 2011.1.12 所有错误用 throw(new Exception('')) 代替
 *
...
class FTP extends Object
{
	 ...
	 * @var array
	 */
	private $_autoAscii = array ("asp", "bat", "c", "cpp", "csv", "h", "htm", "html", "shtml", "ini", "inc", "log", "php", "php3", "pl", "perl", "sh", "sql", "txt", "xhtml", "xml");
	 ...
	 */
	public static function getInstance($host = '', $port = '21', $options = null, $user = null, $pass = null)
	{
		if(empty($host)&&empty($user)&&empty($pass)){
			$config = Config::getConfig("ftp");
	...
	public function connect($host = '127.0.0.1', $port = 21) {
		...
		if (FTP_NATIVE) {
			...
			if ($this->_conn === false) {
				throw(new Exception('FTP::connect: Could not connect to host "'.$host.'" on port '.$port));
				return false;
			}
		...
		if (!$this->_conn) {
			throw(new Exception('FTP::connect: Could not connect to host "'.$host.'" on port '.$port, 'Socket error number '.$errno.' and error message: '.$err));
			return false;
		}
		...
		if (!$this->_verifyResponse(220)) {
			throw(new Exception('FTP::connect: Bad response Server response: '.$this->_response.' [Expected: 220]'));
			return false;
		}
	 ...
	 * @return boolean True if successful
	 */
	public function login($user = 'anonymous', $pass = 'FTP@joomla.org') {

		// If native FTP support is enabled lets use it...
		if (FTP_NATIVE) {
			if (@ftp_login($this->_conn, $user, $pass) === false) {
				throw(new Exception('FTP::login: Unable to login'));
				return false;
			}
			return true;
		}
		...
		if (!$this->_putCmd('USER '.$user, array(331, 503))) {
			throw(new Exception('FTP::login: Bad Username, Server response: '.$this->_response.' [Expected: 331] Username sent: '.$user));
			return false;
		}
		...
		if (!$this->_putCmd('PASS '.$pass, 230)) {
			throw(new Exception('FTP::login: Bad Password Server response: '.$this->_response.' [Expected: 230] Password sent: '.str_repeat('*', strlen($pass))));
			return false;
		}
	...
	public function pwd() {
		...
		if (FTP_NATIVE) {
			if (($ret = @ftp_pwd($this->_conn)) === false) {
				throw(new Exception('FTP::pwd: Bad response'));
				return false;
			}
			return $ret;
		}
		...
		if (!$this->_putCmd('PWD', 257)) {
			throw(new Exception('FTP::pwd: Bad response Server response: '.$this->_response.' [Expected: 257]'));
			return false;
		}
	...
	public function syst() {
		...
		if (FTP_NATIVE) {
			if (($ret = @ftp_systype($this->_conn)) === false) {
				throw(new Exception('FTP::syst: Bad response' ));
				return false;
			}
		} else {
			// Send print working directory command and verify success
			if (!$this->_putCmd('SYST', 215)) {
				throw(new Exception('FTP::syst: Bad response, Server response: '.$this->_response.' [Expected: 215]'));
				return false;
			}
			$ret = $this->_response;
		}
	...
	public function chdir($path) {
		...
		if (FTP_NATIVE) {
			if (@ftp_chdir($this->_conn, $path) === false) {
				throw(new Exception('FTP::chdir: Bad response'));
				return false;
			}
			return true;
		}
		...
		if (!$this->_putCmd('CWD '.$path, 250)) {
			throw(new Exception('FTP::chdir: Bad response, Server response: '.$this->_response.' [Expected: 250] Path sent: '.$path));
			return false;
		}
	...
	public function reinit() {
		...
		if (FTP_NATIVE) {
			if (@ftp_site($this->_conn, 'REIN') === false) {
				throw(new Exception('FTP::reinit: Bad response'));
				return false;
			}
			return true;
		}
		...
		if (!$this->_putCmd('REIN', 220)) {
			throw(new Exception('FTP::reinit: Bad response, Server response: '.$this->_response.' [Expected: 220]'));
			return false;
		}
	...
	public function rename($from, $to) {
		...
		if (FTP_NATIVE) {
			if (@ftp_rename($this->_conn, $from, $to) === false) {
				throw(new Exception('FTP::rename: Bad response'));
				return false;
			}
			return true;
		}
		...
		if (!$this->_putCmd('RNFR '.$from, 350)) {
			throw(new Exception('FTP::rename: Bad response Server response: '.$this->_response.' [Expected: 320] From path sent: '.$from ));
			return false;
		}

		// Send rename to command to the server
		if (!$this->_putCmd('RNTO '.$to, 250)) {
			throw(new Exception('FTP::rename: Bad response, Server response: '.$this->_response.' [Expected: 250] To path sent: '.$to));
			return false;
		}
	...
	public function chmod($path, $mode) {
		...
		if (FTP_NATIVE) {
			if (@ftp_site($this->_conn, 'CHMOD '.$mode.' '.$path) === false) {
				if($this->_OS != 'WIN') {
					throw(new Exception('FTP::chmod: Bad response path:'.$path));
				}
				return false;
			}
			return true;
		...
		if (!$this->_putCmd('SITE CHMOD '.$mode.' '.$path, array(200, 250))) {
			if($this->_OS != 'WIN') {
				throw(new Exception('FTP::chmod: Bad response, Server response: '.$this->_response.' [Expected: 200 or 250] Path sent: '.$path.' Mode sent: '.$mode));
			}
			return false;
		}
		return true;
	...
	public function delete($path) {
		...
		if (FTP_NATIVE) {
			if (@ftp_delete($this->_conn, $path) === false) {
				if (@ftp_rmdir($this->_conn, $path) === false) {
					throw(new Exception('FTP::delete: Bad response path:'.$path));
					return false;
				}
			}
			return true;
		...
		if (!$this->_putCmd('DELE '.$path, 250)) {
			if (!$this->_putCmd('RMD '.$path, 250)) {
				throw(new Exception('FTP::delete: Bad response Server response: '.$this->_response.' [Expected: 250] Path sent: '.$path));
				return false;
			}
		}
		return true;
	...
	/**
	 * Method to delete a path folder on the FTP server
	 *
	 * @access public
	 * @param string $path Path to delete
	 * @return boolean True if successful
	...
	public function rmdir($path){
		$list = $this->listDetails($path);
		if(is_array($list)){
			foreach($list as $item){
				$tmppath = $path."/".$item["name"];
				if($item["type"]=="1"){
					$this->rmdir($tmppath);
				}else{
					$this->delete($tmppath);
	...
	public function mkdir($path) {
		...
		if (FTP_NATIVE) {
			if (@ftp_mkdir($this->_conn, $path) === false) {
				throw(new Exception('FTP::mkdir: Bad response,Path:'.$path));
				return false;
			}
			return true;
		}
		...
		if (!$this->_putCmd('MKD '.$path, 257)) {
			throw(new Exception('FTP::mkdir: Bad response Server response: '.$this->_response.' [Expected: 257] Path sent: '.$path));
			return false;
		}
		return true;
	}
	...
	public function restart($point) {
		...
		if (FTP_NATIVE) {
			if (@ftp_site($this->_conn, 'REST '.$point) === false) {
				throw(new Exception('FTP::restart: Bad response'));
				return false;
			}
			return true;
		}
		...
		if (!$this->_putCmd('REST '.$point, 350)) {
			throw(new Exception('FTP::restart: Bad response Server response: '.$this->_response.' [Expected: 350] Restart point sent: '.$point));
			return false;
		}
	...
	public function create($path) {
		...
		if (FTP_NATIVE) {
			...
			if (@ftp_pasv($this->_conn, true) === false) {
				throw(new Exception('FTP::create: Unable to use passive mode' ));
				return false;
			}

			$buffer = fopen('buffer://tmp', 'r');
			if (@ftp_fput($this->_conn, $path, $buffer, FTP_ASCII) === false) {
				throw(new Exception('FTP::create: Bad response' ));
				fclose($buffer);
				return false;
			}
			fclose($buffer);
		...
		if (!$this->_putCmd('STOR '.$path, array (150, 125))) {
			...
			throw(new Exception('FTP::create: Bad response, Server response: '.$this->_response.' [Expected: 150 or 125] Path sent: '.$path ));
			return false;
		}
		...
		if (!$this->_verifyResponse(226)) {
			throw(new Exception('FTP::create: Transfer Failed, Server response: '.$this->_response.' [Expected: 226] Path sent: '.$path ));
			return false;
		}
	...
	public function read($remote, $buffer) {
		...
		if (FTP_NATIVE) {
			// turn passive mode on
			if (@ftp_pasv($this->_conn, true) === false) {
				throw(new Exception('FTP::read: Unable to use passive mode' ));
				return false;
			}
			...
			if (@ftp_fget($this->_conn, $tmp, $remote, $mode) === false) {
				fclose($tmp);
				throw(new Exception('FTP::read: Bad response' ));
				return false;
			}
		...
		if (!$this->_putCmd('RETR '.$remote, array (150, 125))) {
			@ fclose($this->_dataconn);
			throw(new Exception('FTP::read: Bad response, Server response: '.$this->_response.' [Expected: 150 or 125] Path sent: '.$remote ));
			return false;
		}
		...
		if (!$this->_verifyResponse(226)) {
			throw(new Exception('FTP::read: Transfer Failed, Server response: '.$this->_response.' [Expected: 226] Path sent: '.$remote ));
			return false;
		}
	...
	public function get($local, $remote) {
		...
		if (FTP_NATIVE) {
			// turn passive mode on
			if (@ftp_pasv($this->_conn, true) === false) {
				throw(new Exception('FTP::get: Unable to use passive mode' ));
				return false;
			}

			if (@ftp_get($this->_conn, $local, $remote, $mode) === false) {
				throw(new Exception('FTP::get: Bad response' ));
				return false;
			}
			return true;
		}
		...
		$fp = fopen($local, "wb");
		if (!$fp) {
			throw(new Exception('FTP::get: Unable to open local file for writing', 'Local path: '.$local ));
			return false;
		}
		...
		if (!$this->_putCmd('RETR '.$remote, array (150, 125))) {
			@ fclose($this->_dataconn);
			throw(new Exception('FTP::get: Bad response, Server response: '.$this->_response.' [Expected: 150 or 125] Path sent: '.$remote ));
			return false;
		}
		...
		if (!$this->_verifyResponse(226)) {
			throw(new Exception('FTP::get: Transfer Failed, Server response: '.$this->_response.' [Expected: 226] Path sent: '.$remote ));
			return false;
		}
	...
	public function store($local, $remote = null) {
		...
		if (FTP_NATIVE) {
			// turn passive mode on
			if (@ftp_pasv($this->_conn, true) === false) {
				throw(new Exception('FTP::store: Unable to use passive mode' ));
				return false;
			}

			if (@ftp_put($this->_conn, $remote, $local, $mode) === false) {
				throw(new Exception('FTP::store: Bad response' ));
				return false;
			}
			return true;
		}
		...
		if (@ file_exists($local)) {
			$fp = fopen($local, "rb");
			if (!$fp) {
				throw(new Exception('FTP::store: Unable to open local file for reading, Local path: '.$local ));
				return false;
			}
		} else {
			throw(new Exception('FTP::store: Unable to find local path, Local path: '.$local ));
			return false;
		}
		...
		if (!$this->_putCmd('STOR '.$remote, array (150, 125))) {
			@ fclose($fp);
			@ fclose($this->_dataconn);
			throw(new Exception('FTP::store: Bad response, Server response: '.$this->_response.' [Expected: 150 or 125] Path sent: '.$remote ));
			return false;
		}
		...
		while (!feof($fp)) {
			...
			do {
				if (($result = @ fwrite($this->_dataconn, $line)) === false) {
					throw(new Exception('FTP::store: Unable to write to data port socket' ));
					return false;
				}
		...
		if (!$this->_verifyResponse(226)) {
			throw(new Exception('FTP::store: Transfer Failed, Server response: '.$this->_response.' [Expected: 226] Path sent: '.$remote ));
			return false;
		}
	...
	public function write($remote, $buffer) {
		...
		if (FTP_NATIVE) {
			// turn passive mode on
			if (@ftp_pasv($this->_conn, true) === false) {
				throw(new Exception('FTP::write: Unable to use passive mode' ));
				return false;
			}
			...
			if (@ftp_fput($this->_conn, $remote, $tmp, $mode) === false) {
				fclose($tmp);
				throw(new Exception('FTP::write: Bad response' ));
				return false;
			}
			fclose($tmp);
			return true;
		...
		// Send store command to the FTP server
		if (!$this->_putCmd('STOR '.$remote, array (150, 125))) {
			throw(new Exception('FTP::write: Bad respons, Server response: '.$this->_response.' [Expected: 150 or 125] Path sent: '.$remote ));
			@ fclose($this->_dataconn);
			return false;
		}
		...
		do {
			if (($result = @ fwrite($this->_dataconn, $buffer)) === false) {
				throw(new Exception('FTP::write: Unable to write to data port socket' ));
				return false;
			}
		...
		if (!$this->_verifyResponse(226)) {
			throw(new Exception('FTP::write: Transfer Failed, Server response: '.$this->_response.' [Expected: 226] Path sent: '.$remote ));
			return false;
		}
	...
	public function listNames($path = null) {
		...
		if (FTP_NATIVE) {
			// turn passive mode on
			if (@ftp_pasv($this->_conn, true) === false) {
				throw(new Exception('FTP::listNames: Unable to use passive mode' ));
				return false;
			}
			...
			if (($list = @ftp_nlist($this->_conn,$path)) === false) {
				...
				throw(new Exception('FTP::listNames: Bad response' ));
				return false;
			}
		...
		if (!$this->_putCmd('NLST'.$path, array (150, 125))) {
			...
			throw(new Exception('FTP::listNames: Bad response, Server response: '.$this->_response.' [Expected: 150 or 125] Path sent: '.$path ));
			return false;
		}
		...
		// Everything go okay?
		if (!$this->_verifyResponse(226)) {
			throw(new Exception('FTP::listNames: Transfer Failed, Server response: '.$this->_response.' [Expected: 226] Path sent: '.$path ));
			return false;
		}
	...
	public function listDetails($path = null, $type = 'all') {
		...
		if (FTP_NATIVE) {
			// turn passive mode on
			if (@ftp_pasv($this->_conn, true) === false) {
				throw(new Exception('FTP::listDetails: Unable to use passive mode' ));
				return false;
			}

			if (($contents = @ftp_rawlist($this->_conn, $path)) === false) {
				throw(new Exception('FTP::listDetails: Bad response' ));
				return false;
			}
		} else {
			// Non Native mode
			...
			if (!$this->_putCmd(($recurse == true) ? 'LIST -R' : 'LIST'.$path, array (150, 125))) {
				throw(new Exception('FTP::listDetails: Bad response, Server response: '.$this->_response.' [Expected: 150 or 125] Path sent: '.$path ));
				@ fclose($this->_dataconn);
				return false;
			}
			...
			// Everything go okay?
			if (!$this->_verifyResponse(226)) {
				throw(new Exception('FTP::listDetails: Transfer Failed, Server response: '.$this->_response.' [Expected: 226] Path sent: '.$path ));
				return false;
			}
		...
		if (!$osType) {
			throw(new Exception('FTP::listDetails: Unrecognized directory listing format' ));
			return false;
		}
	...
	/**
	 * Check folder or file is in listNames.
	 *
	 * @access public
	 * @param string $path Path local file to check on the FTP server
	 * @return boolean True/False
	 */
	public function exists($path){
		$list = $this->listNames(dirname($path));
		if (is_array($list)){ 
			if (in_array(basename($path), $list)) 
			{ 
				return true;
			}
		}
	...
	private function _putCmd($cmd, $expectedResponse) {
		...
		if (!is_resource($this->_conn)) {
			throw(new Exception('FTP::_putCmd: Not connected to the control port' ));
			return false;
		}

		// Send the command to the server
		if (!fwrite($this->_conn, $cmd."\r\n")) {
			throw(new Exception('FTP::_putCmd: Unable to send command: '.$cmd ));
		}
	...
	private function _verifyResponse($expected) {
		...
		if (!isset($parts[1])) {
			throw(new Exception('FTP::_verifyResponse: Timeout or unrecognized response while waiting for a response from the server, Server response: '.$this->_response));
			return false;
		}
	...
	private function _passive() {
		...
		// Make sure we have a connection to the server
		if (!is_resource($this->_conn)) {
			throw(new Exception('FTP::_passive: Not connected to the control port' ));
			return false;
		}
		...
		// Catch a timeout or bad response
		if (!isset($parts[1])) {
			throw(new Exception('FTP::_passive: Timeout or unrecognized response while waiting for a response from the server, Server response: '.$this->_response));
			return false;
		}
		...
		if ($this->_responseCode != '227') {
			throw(new Exception('FTP::_passive: Unable to obtain IP and port for data transfer, Server response: '.$this->_responseMsg));
			return false;
		}

		// Snatch the IP and port information, or die horribly trying...
		if (preg_match('~\((\d+),\s*(\d+),\s*(\d+),\s*(\d+),\s*(\d+)(?:,\s*(\d+))\)~', $this->_responseMsg, $match) == 0) {
			throw(new Exception('FTP::_passive: IP and port for data transfer not valid, Server response: '.$this->_responseMsg));
			return false;
		}
		...
		if (!$this->_dataconn) {
			throw(new Exception('FTP::_passive: Could not connect to host '.$this->_pasv['ip'].' on port '.$this->_pasv['port'].'.  Socket error number '.$errno.' and error message: '.$err ));
			return false;
		}
	...
	private function _mode($mode) {
		if ($mode == FTP_BINARY) {
			if (!$this->_putCmd("TYPE I", 200)) {
				throw(new Exception('FTP::_mode: Bad response, Server response: '.$this->_response.' [Expected: 200] Mode sent: Binary' ));
				return false;
			}
		} else {
			if (!$this->_putCmd("TYPE A", 200)) {
				throw(new Exception('FTP::_mode: Bad response, Server response: '.$this->_response.' [Expected: 200] Mode sent: Ascii' ));
				return false;
			}
		}
		return true;
