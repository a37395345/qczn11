...
if (!defined('PATH_SITE')) {
	define('PATH_SITE',Config::homedir());
}
if (!defined('PATH_ROOT')) {
	/** string The root directory of the file system in native format */
	define('PATH_ROOT', Path::clean(PATH_SITE));
}
...
class Path
{
	...
	public static function check($path)
	{
		if (strpos($path, '..') !== false) {
			throw(new Exception('20,Path::check Use of relative paths not permitted'));
			exit();
		}
		$path = Path::clean($path);
		if (strpos($path, Path::clean(Config::basedir())) !== 0) {
			throw(new Exception('20,Path::check Snooping out of bounds @ '.$path));
			exit();
		}
	}
