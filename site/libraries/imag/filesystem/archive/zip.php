 ...
 * 
 * $list = array(
 * 	array('path'=>'E:\mywork\code\autoCode\slice\service\test.txt','dir'=>'dir\base'),
 * 	array('path'=>'E:\mywork\code\autoCode\slice\service\test.doc','dir'=>'dir\base2'),
 * );

 * $zip = Archive::getAdapter("zip");
 * $zip->create('E:\mywork\code\autoCode\slice\service\test.zip',$list);
 * 
 */
class ArchiveZip extends Object
{
	/**
	 * ZIP compression metho"/".
	 * @var array
	 */
	...
	public function create($archive, $files, $options = array ())
	{
		...
		{
			$flist = array('data'=>FuseFile::read($file["path"]),'name'=>$file["dir"].DS.basename($file["path"]));
			$this->_addToZIPFile($flist, $contents, $ctrldir);
		}
	...
	public function extract($archive, $destination, $options = array ())
	{
		...
		if ($this->hasNativeSupport()) {
			return ($this->_extractNative($archive, $destination, $options))? true : new Exception($this->get('error.message'));
		} else {
			return ($this->_extract($archive, $destination, $options))? true : new Exception($this->get('error.message'));
		}
	}
	...
	private function _extract($archive, $destination, $options)
	{
		...
		for ($i=0,$n=count($this->_metadata);$i<$n;$i++) {
			if (substr($this->_metadata[$i]['name'], -1, 1) != '/' && substr($this->_metadata[$i]['name'], -1, 1) != '\\') {
				...
				if (FuseFile::write($path, $buffer) === false) {
				...
				while ($file = @zip_read($zip))
				{
					if (zip_entry_open($zip, $file, "r")) {
						if (substr(zip_entry_name($file), strlen(zip_entry_name($file)) - 1) != "/") {
							...
							if (FuseFile::write($destination."/".zip_entry_name($file), $buffer) === false) {
								$this->set('error.message', 'Unable to write entry');
								return false;
							}
	 ...
	 * </pre>
	 */
	private function _getZipInfo($data)
	{
		// Initialize variables
		$entries = array ();
		...
		do {
			...
			$entries[$name] = array('attr' => null, 'crc' => sprintf("%08s", dechex($info['CRC32'] )), 'csize' => $info['Compressed'], 'date' => null, '_dataStart' => null, 'name' => $name, 'method' => $this->_method[$info['Method']], '_method' => $info['Method'], 'size' => $info['Uncompressed'], 'type' => null);
	...
	private function _aadToZipFolder($contents,$ctrldir){
		
	}

	/**
	 * Ad"/" a "file" to the ZIP archive.
	 *
