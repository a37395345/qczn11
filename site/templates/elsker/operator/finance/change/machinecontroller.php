...
class MachineController extends AdminController
{
	...
	function create(){	
        ...
        $view  = $this->createView("operator/machine/create.html");
		$view->assign($object);
		$view->display();
	}
	...
	function insert(){	
		...
		if(empty($car_num)){
			print_r("还说自己点击不是添加！");
			Components::save_admin_log("故意找事情！");
			exit;
		}
		...
		$object = new stdClass();
	  	...
	  	$object->car_type=Request::getVar('car_type','post');
	  	...
	  	//$object->car_position=Request::getVar('car_position','post');
	  	//$car_positionDate=Request::getVar('car_positionDate','post');
	  	//$object->car_positionDate=empty($car_positionDate)?0:strtotime($car_positionDate);
	  	//$car_nav=Request::getVar('car_nav','post');
	  	//$object->car_nav=empty($car_nav)?0:$car_nav;
	  	$object->car_remarks=Request::getVar('car_remarks','post');
	  	$object->car_remarks_a=Request::getVar('car_remarks_a','post');
	  	$object->car_remarks_b=Request::getVar('car_remarks_b','post');
	...
	function update(){	
		...
		$arrDelimg=Request::getVar('delimg','post');
	  	...
	  	$object->car_remarks_a=Request::getVar('car_remarks_a','post');
	  	$object->car_remarks_b=Request::getVar('car_remarks_b','post');
	  	//print_r($object->car_remarks_a);exit;
	  	//$object->car_status=Request::getVar('car_status','post');
	  	$object->car_price=Request::getVar('car_price','post');
	  	$object->car_image=$car_image;
		...
		if ($model->update($object,'car_id'))
		{
			if (!empty($arrDelimg)){
				foreach($arrDelimg as $key=>$val){
					$model->delete2($val,"car_images","id");
				}
	...
	function price(){
		...
		$pricelist = $model->getCarPrice($car_id);
		...
		$object->total= $total;
		if (!empty($id)){
		$object->priceinfo = $model->getInfo($id,"car_price","id");
		}
	...
	function price_a(){
		...
		$pricelist = $model->getListBySql_a($sql);
        ...
        $view  = $this->createView("operator/machine/price_a.html");
		$view->assign($object);
		$view->display();
	}
	...
	function price_acc(){
		...
		$object->car_id= $car_id;
		$object->plan_name= Request::getVar('plan_name','post');
		$aa=Request::getVar('plan_day','post');
		$object->plan_day=(empty($aa) ? 0 : $aa);
		$aa=Request::getVar('plan_rent','post');
		$object->plan_rent=(empty($aa) ? 0 : $aa);
		$aa=Request::getVar('plan_rentamount','post');
		$object->plan_rentamount=(empty($aa) ? 0 : $aa);
		$aa=Request::getVar('plan_deposit1','post');
		$object->plan_deposit1=(empty($aa) ? 0 : $aa);
		$aa=Request::getVar('plan_deposit2','post');
		$object->plan_deposit2=(empty($aa) ? 0 : $aa);
		$aa=Request::getVar('start_Date','post');
		$object->start_Date=(empty($aa) ? 0 : strtotime($aa));
		$aa=Request::getVar('end_Date','post');
		$object->end_Date=(empty($aa) ? 0 : strtotime($aa));
		...
		$object->plan_rentRemarks= Request::getVar('plan_rentRemarks','post');
		if (!empty($id)){
			$model->update($object,'id',"car_price");
		}else{
			$id=$model->store($object,"car_price");
		}
		$minprice=$model->getTotal("Select min(plan_rentamount) as total from car_price where car_id={$car_id} and plan_day=1");
		$object = new stdClass();
		$object->car_id=$car_id;
		$object->car_price=$minprice;
