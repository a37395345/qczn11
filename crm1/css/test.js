    ...
    function gettime(){
       ...
       var e_time=timestampToTime(time+29*60*60*24);
       ...
       $("#paiche_startDate").val(timestampToTime(time+9*60*60*24));
       xuanran();
    }
  ...
  function xuanran(){
    ...
    if(list.length>0){
      for(var i=0;i<list.length;i++){
              var html_a="<tr><th data-status="+list[i]['car_status']+" data-maintreason="+list[i]['car_maintreason']+" width='10%'>苏D"+list[i]['car_num']+"<div class='contact-box'><div class='col-sm-6 car_img' style='width:206px;height:165px;'><div class='text-center'><img src=../../../thumb/"+list[i]['car_image']+"><div style='color:#000;' class='car_name1'>"+list[i]['car_model']+"</div></div></div><div class='col-sm-6 car_txt'><h4>苏D"+list[i]['car_num']+"</h4><p>状态：<span></span></p><p>租金："+list[i]['plan_rentamount']+"元/天</p><p>老客户押金："+list[i]['plan_deposit1']+"元/天</p><p>新客户押金："+list[i]['plan_deposit2']+"元/天</p></div></div></th><td width='90%' class='tdfirst td2401' day='24'>";
              //显示开始时间
              var s_timea=time;
              //显示结束的时间
              ...
              $.each(list[i]["time"], function (k, item) {
                $.each(item, function (j, item_a) {
                      ...
                      if(item_a.hasOwnProperty("shop_name")){
                      ...
                      +"<div class='car_content'>"+
                      "<div class='car_txt1'>"+
                        "<h3 class='car_xq'>车辆详情</h3><p>类型："+car_lx+"<br>业务门店：总公司<br/>租车开始时间："+item_a['paiche_startDate']
                          +"<br>租车结束时间："+item_a['paiche_endDate']
                        +"</p></div></div></span>";
                      }
                      // html_a+="<span style='z-index:9;top:8px;left:"+left+"%;width:"+width+"%;height:30px;background-color:"+bcolor+";'>"
    ...
    $(function(){
      $(".car tr th").each(function(){
        if($(this).attr("data-status")==2){
          if($(this).attr("data-maintreason")==0){
            $(this).addClass("gzwx");
            $(this).find("span").text("故障维修");
          }
          if($(this).attr("data-maintreason")!=0){
            $(this).addClass("bp");
            $(this).find("span").text("异常/被骗");
          }
        }else if($(this).attr("data-status")!=2){
          $(this).find("span").text("正常");
        }
      });
