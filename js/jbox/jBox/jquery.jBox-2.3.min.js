(function(a) {
	a.jBox = function(b, c) {
		...
		j.push('<a class="jbox-close" title="' + a.jBox.languageDefaults.close + '" onmouseover="$(this).addClass(\'jbox-close-hover\');" onmouseout="$(this).removeClass(\'jbox-close-hover\');" style="position:absolute; display:block; cursor:pointer; top:' + (0x16 + c.border - 10) + 'px; right:' + (0x16 + c.border) + 'px; width:15px; height:15px;' + (c.showClose ? '': 'display:none;') + '"></a>');
		if (g) {
			j.push('<div class="jbox-title-panel" style="height:35px;">');
			j.push('<div class="jbox-title' + (c.showIcon == true ? ' jbox-title-icon': (c.showIcon == false ? '': ' ' + c.showIcon)) + '" style="float:left; width:' + i + '; line-height:35px; padding-left:' + (c.showIcon ? 0x12: 0x5) + 'px;overflow:hidden;text-overflow:ellipsis;word-break:break-all;text-align:center;color:#353333">' + (c.title == '' ? '&nbsp;': c.title) + '</div>');
			j.push('</div>')
		};
