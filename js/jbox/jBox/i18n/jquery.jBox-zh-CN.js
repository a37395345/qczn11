...
jBoxConfig.defaults = {
    ...
    top: '2%', /* 窗口离顶部的距离,可以是百分比或像素(如 '100px') */

    border: 3, /* 窗口的外边框像素大小,必须是0以上的整数 */

    opacity: 0.6, /* 窗口隔离层的透明度,如果设置为0,则不显示隔离层 */
...
jBoxConfig.messagerDefaults = {
    ...
    width: 'auto', /* 信息的高度，值为'auto'或表示像素的整数 */

    height: 'auto', /* 信息的高度，值为'auto'或表示像素的整数 */

    timeout: 1000, /* 信息显示多少毫秒后自动关闭,如果设置为0,则不自动关闭 */
...
function demo_iframe(url,title,w,h,id) {

	try{

    $.jBox.open("iframe:"+url, title, w, h, { buttons: { '关闭': true},id: id });

	}

	catch(a){

    jQuery.jBox.open("iframe:"+url, title, w, h, { buttons: { '关闭': true},id: id });
