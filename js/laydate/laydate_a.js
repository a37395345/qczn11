...
function(a) {
	var myDate_a = new Date();

    //获取当前年
   var year_a = myDate_a.getFullYear();

   //获取当前月
   var month_a = myDate_a.getMonth() + 1;
	month_a=month_a < 10 ? '0' + month_a : month_a;
    //获取当前日
    var date_a = myDate_a.getDate();
    date_a=date_a < 10 ? '0' + date_a : date_a;
    var ha = myDate_a.getHours(); //获取当前小时数(0-23)
    ha=ha < 10 ? '0' + ha : ha;

    var ma = myDate_a.getMinutes(); //获取当前分钟数(0-59)
    ma=ma < 10 ? '0' + ma : ma;
    var sa = myDate_a.getSeconds();
    sa=sa < 10 ? '0' + sa : sa;

   //获取当前时间
 var now_a = year_a + '-' + month_a + "-" + date_a + " " + ha + ':' + ma + ":" + sa;
